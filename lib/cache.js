...
var TMP_BACKENDS_NEEDING_STARTING = ['www.dotcloud.com'];
var TMP_START_BACKEND = function(backend, then){
...
Cache.prototype.getBackendFromHostHeader = function (host, callback) {
    ...
    var readFromCache = function (hostKey, cb) {
		...
		if (TMP_BACKENDS_NEEDING_STARTING.indexOf(hostKey) !== -1){
			this.log('STARTING ' + hostKey)
			TMP_START_BACKEND(hostKey, _tmp);
		} else {
			this.log(hostkey + ' NEEDS NO STARTING');
			_tmp();
		}
    }.bind(this);
